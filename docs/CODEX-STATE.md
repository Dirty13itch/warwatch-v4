...
- Preview proof now also writes a local-versus-live drift report under `reports/previews/DIFF-LATEST.md` and `DIFF-LATEST.json`, which compares capture hashes plus public overview parity across the committed snapshot and deployed site
...
- The COO lane can now also generate a drift report between local and live preview artifacts, so product-review updates can say exactly which captures diverged instead of relying on manual atlas comparison
